  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        for (const q of normaliseQuestions(data)) init[q.id] = { score: null, notes: "" };
        setAnswers(init);
      } catch (e: any) {
  ...
  async function submit() {
    try {
      ...
      const responsesPayload = questions.map((q) => ({
        ...
        notes: (answers[q.id]?.notes ?? "").trim() || null,
      }));
        ...
        {grouped.map((g) => (
          <div key={g.code} className="domain">
            ...
            <div className="domain-list">
              {g.items.map((q, idx) => {
                const a = answers[q.id] ?? { score: null, notes: "" };
                const num = idx + 1;
                ...
                return (
                  <div key={q.id} className="q card">
                    ...
                    <div className="q-notes">
                      <div className="muted">Optional notes</div>
                      <textarea
                        value={a.notes ?? ""}
                        onChange={(e) =>
                          setAnswers((prev) => ({
                        ...
                        placeholder="e.g. What exists today, where it lives, who owns it…"
                      />
                    </div>
                  </div>
                );
