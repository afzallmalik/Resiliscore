// EXECUTIVE VISUAL POLISH - Resiliscore assurance theme
...
import { getImplementationGuidance } from "@/lib/implementationLayer";
...
const BRAND = {
  // Backgrounds
  bg: rgb(0.972, 0.982, 0.988),
  white: rgb(1, 1, 1),
  card: rgb(0.985, 0.99, 0.995),

  // Header / primary brand — executive assurance tone
  headerBg: rgb(0.024, 0.106, 0.133),     // deep navy / Resiliscore
  headerGlow: rgb(0.133, 0.827, 0.933),   // restrained cyan

  // Core accent
  accent: rgb(0.133, 0.827, 0.933),       // cyan
  accentSoft: rgb(0.88, 0.97, 0.985),
  accentLine: rgb(0.64, 0.86, 0.90),

  // Text
  text: rgb(0.055, 0.075, 0.105),
  muted: rgb(0.38, 0.44, 0.50),

  // Structure
  line: rgb(0.84, 0.88, 0.91),

  // Status
  good: rgb(0.08, 0.72, 0.65),
  med: rgb(0.94, 0.62, 0.08),
  risk: rgb(0.90, 0.26, 0.26),

  // Tinted surfaces
  greenTint: rgb(0.92, 0.985, 0.975),
  amberTint: rgb(0.998, 0.972, 0.90),
  redTint: rgb(0.995, 0.935, 0.935),
  blueTint: rgb(0.92, 0.975, 0.99),
};
...
function drawExplanatoryPlanItem(
  ...
  font: any,
  fontBold: any,
  markerColor?: ReturnType<typeof rgb>
) {
  let cy = y;
  const titleLines = wrapText(item, widthChars);
  const explainer = getActionExplanation(item, phase);
  const explanationLines = wrapText(explainer, widthChars - 6);

  const blockHeight = Math.max(42, titleLines.length * 13 + explanationLines.length * 11 + 18);

  // Subtle circular marker to break up dense action text without changing content.
  page.drawCircle({
    x: x + 5,
    y: cy - 4,
    size: 4,
    color: markerColor ?? BRAND.accent,
    opacity: 0.9,
  });

  let textY = cy;
  for (let i = 0; i < titleLines.length; i++) {
    page.drawText(sanitizeText(titleLines[i]), {
      x: x + 18,
      y: textY,
      size: 10.2,
      font: fontBold,
  ...
  // Light divider between plan actions.
  page.drawLine({
    start: { x: x + 18, y: cy - blockHeight + 7 },
    end: { x: 545, y: cy - blockHeight + 7 },
    thickness: 0.55,
    color: BRAND.line,
    opacity: 0.55,
  });

  return cy - blockHeight;
}
 ...
 */
{
  ...
  page.drawText("Assessment details", {
    ...
    size: 8.5,
    font: fontBold,
    color: rgb(0.24, 0.32, 0.38),
  });

  let infoY = height - 282;
  const detailLabelColor = rgb(0.36, 0.45, 0.52);
  const detailValueColor = rgb(0.06, 0.09, 0.12);
  ...
  const drawRagGroup = (
    ...
    if (!domains.length) {
      ...
      for (const d of domains) {
        ...
        // Subtle RAG marker to make each domain easier to scan without changing report content.
        page.drawCircle({ x: 55, y: y + 4, size: 3.5, color, opacity: 0.9 });
        page.drawText(sanitizeText(name), {
          x: 66,
          y,
          size: 10.5,
 ...
 */
{
  ...
  const startNewPlanPage = (continued = false) => {
    drawFooter(page, pageNum++, font, reportRef);
    page = addPage(pdfDoc);
    drawBrandHeader(page, shieldImg);
    drawWatermark(page, watermarkImg, 0.12);
    ...
    drawSectionTitle(
      page,
      continued ? "30 / 60 / 90 Day Improvement Plan - Continued" : "30 / 60 / 90 Day Improvement Plan",
      50,
      y,
      fontBold
    );
  ...
  const ensurePlanSpace = (needed: number) => {
    if (y - needed < 110) {
      startNewPlanPage(true);
    }
  };
  ...
  const intro =
    "This plan is designed to reduce real-world disruption risk as quickly as possible. Start with the priority areas shown in the RAG view, then use the phases below in order: stabilise first, prove controls second, and embed evidence and measurement third.";
  for (const line of wrapText(intro, 100)) {
    page.drawText(line, {
      x: 50,
      y,
  ...
  for (let blockIndex = 0; blockIndex < blocks.length; blockIndex++) {
    ...
    // Keep Phase 01 on the opening plan page after the introduction.
    // Start Phase 02 and Phase 03 on their own pages for stronger executive pacing.
    if (blockIndex > 0) {
      drawFooter(page, pageNum++, font, reportRef);
      page = addPage(pdfDoc);
      drawBrandHeader(page, shieldImg);
      ...
      drawSectionTitle(
        page,
        "30 / 60 / 90 Day Improvement Plan - Continued",
        50,
        y,
        fontBold
      );
    ...
    const subtitleLines = wrapText(b.subtitle, 86);
    ensurePlanSpace(82 + subtitleLines.length * 13);

    // Executive-style phase container inspired by the AIRA action plan structure.
    const phaseHeaderH = 68 + subtitleLines.length * 12;

    drawRoundedCard(page, 50, y - phaseHeaderH + 8, 495, phaseHeaderH, {
      fill: rgb(0.985, 0.988, 0.992),
      border: BRAND.line,
      radius: 18,
    });
    ...
    // Small executive phase label
    page.drawText(`IMPLEMENTATION PHASE ${b.marker}`, {
      x: 108,
      y: y - 10,
      size: 8.8,
      font: fontBold,
    ...
    for (const item of b.items) {
      const explanationLines = wrapText(getActionExplanation(item, b.phase), 82);

      const needed =
        Math.max(
          60,
          explanationLines.length * 11 + 34
        ) + 18;

      ensurePlanSpace(needed);

      y = drawExplanatoryPlanItem(
        page,
        item,
        b.phase,
        62,
 ...
 */
{
  ...
  const intro =
    "Use this page immediately after the 30/60/90 plan. It translates the roadmap into ownership, review points, evidence expectations, and practical questions leadership should ask first.";
  for (const line of wrapText(intro, 100)) {
    page.drawText(line, {
      x: 50,
      y,
 ...
 */
{
  ...
  for (const d of focusDomains) {
    ...
    const scenarioLines = wrapText(detail.scenario, 84);
    const exposureLines = wrapText(detail.exposure, 84);
    const fixLines = wrapText(detail.fastestFix, 84);
    const cardHeight = Math.max(206, 76 + scenarioLines.length * 12 + exposureLines.length * 12 + fixLines.length * 12 + 42);
    ...
    if (y - cardHeight < 112) {
      ...
      drawWatermark(page, watermarkImg, 0.12);
      y = height - 96;
      drawSectionTitle(page, "Likely failure scenarios and exposure - Continued", 50, y, fontBold);
      y -= 28;
    }
  ...
  const getSafeImplementationGuidance = (domainName: string, domainCode?: string) => {
    const key = `${domainName} ${domainCode || ""}`.toLowerCase();
    const base = getImplementationGuidance(`${domainName} ${domainCode || ""}`) as any;
...
/**
     ...
     */
    {
      ...
      const { height } = page.getSize();
      let y = height - 96;
      drawSectionTitle(page, "Why these areas matter most", 50, y, fontBold);
      y -= 26;
 ...
 */
{
  ...
  const startFixPage = (title: string) => {
    drawFooter(page, pageNum++, font, reportRef);
    page = addPage(pdfDoc);
    drawBrandHeader(page, shieldImg);
    drawWatermark(page, watermarkImg, 0.08);
  ...
  drawSectionTitle(page, "How to Fix This (Practical Options)", 50, y, fontBold);
  y -= 28;

  const intro =
    "This section turns the weakest areas into practical implementation guidance. It does not recommend vendors. It shows the control required, the technology category that normally supports it, and the realistic SME approach.";
  ...
  const drawOpenImplementationBlock = (d: any, firstOnPage: boolean) => {
    const name = d.domain_name || d.domain_code || "Domain";
    const score = Number(d.score ?? 0);
    const guidance = getSafeImplementationGuidance(name, d.domain_code) as any;

    if (!firstOnPage) {
      startFixPage("How to Fix This (Practical Options) - Continued");
    }
    ...
    y -= 18;
    page.drawRectangle({ x: 50, y, width: 170, height: 5, color: BRAND.accent, opacity: 0.9 });
    page.drawLine({ start: { x: 50, y: y - 12 }, end: { x: 545, y: y - 12 }, thickness: 1, color: BRAND.line });
    y -= 34;

    for (const line of wrapText(guidance.plainEnglish, 94).slice(0, 5)) {
      page.drawText(sanitizeText(line), { x: 50, y, size: 10.2, font, color: BRAND.text });
      y -= 13;
    }

    y -= 10;

    const drawGuidanceSection = (label: string, bodyLines: string[], opts?: { boldBody?: boolean }) => {
      if (y < 120) startFixPage("How to Fix This (Practical Options) - Continued");

      page.drawText(label, { x: 50, y, size: 10.5, font: fontBold, color: BRAND.text });
      y -= 15;

      for (const line of bodyLines) {
        if (y < 92) startFixPage("How to Fix This (Practical Options) - Continued");
        page.drawText(sanitizeText(line), {
          x: 64,
          y,
          size: 10,
          font: opts?.boldBody ? fontBold : font,
          color: BRAND.text,
        });
        y -= 13;
      }
    ...
    drawGuidanceSection("Control required", wrapText(guidance.controlRequired, 90).slice(0, 3));
    drawGuidanceSection(
      "Technology options",
      guidance.technologyCategories.slice(0, 4).flatMap((item: string) => wrapText(`- ${item}`, 88))
    );
    drawGuidanceSection(
      "What to ask for",
      wrapText(guidance.technologyAsk || "Ask your provider what capability, coverage, ownership, reporting, and evidence they will put in place.", 90).slice(0, 6)
    );
    drawGuidanceSection("Typical SME approach", wrapText(guidance.typicalSmeApproach, 90).slice(0, 6));
    drawGuidanceSection(
      "Effort / cost / impact",
      [`${guidance.effort} effort | ${guidance.cost} | ${guidance.maturityImpact}`],
      { boldBody: true }
    );
  };

  ranked.slice(0, 3).forEach((d, index) => {
    drawOpenImplementationBlock(d, index === 0);
  });
...
/**
 * ------------------------------
 * Premium page 4: Technology Implementation Snapshot
 * ------------------------------
 */
{
  let page = addPage(pdfDoc);
  ...
  const startTechPage = (continued = true) => {
    drawFooter(page, pageNum++, font, reportRef);
    page = addPage(pdfDoc);
    drawBrandHeader(page, shieldImg);
    drawWatermark(page, watermarkImg, 0.08);
    ...
    drawSectionTitle(
      page,
      continued ? "Technology Implementation Snapshot - Continued" : "Technology Implementation Snapshot",
      50,
      y,
      fontBold
    );
  ...
  drawSectionTitle(page, "Technology Implementation Snapshot", 50, y, fontBold);
  y -= 28;

  const intro =
    "Use this section before speaking to an IT provider, MSP, or technology vendor. It does not recommend suppliers. It explains the capability to ask for and the evidence that should come back.";
  ...
  const snapshotDomains = ranked.slice(0, 4);

  snapshotDomains.forEach((d, index) => {
    if (index > 0) startTechPage(true);

    const name = d.domain_name || d.domain_code || "Domain";
    const guidance = getSafeImplementationGuidance(name, d.domain_code) as any;

    page.drawText(sanitizeText(name), { x: 50, y, size: 14, font: fontBold, color: BRAND.text });
    page.drawText(`${guidance.effort} effort | ${guidance.cost}`, {
      x: 418,
      y,
      size: 9.5,
      font: fontBold,
    ...
    for (const line of wrapText(guidance.plainEnglish, 94).slice(0, 5)) {
      page.drawText(sanitizeText(line), { x: 50, y, size: 10.1, font, color: BRAND.text });
      y -= 13;
    }

    y -= 10;

    const drawProviderSection = (label: string, lines: string[]) => {
      if (y < 120) startTechPage(true);
      page.drawText(label, { x: 50, y, size: 10.5, font: fontBold, color: BRAND.text });
      y -= 15;

      for (const line of lines) {
        if (y < 92) startTechPage(true);
        page.drawText(sanitizeText(line), { x: 64, y, size: 9.7, font, color: BRAND.text });
        y -= 12.5;
      }
    ...
    drawProviderSection(
      ...
      [guidance.controlRequired, ...guidance.technologyCategories.slice(0, 4)].flatMap((item: string) => wrapText(`- ${item}`, 90)).slice(0, 8)
    );

    drawProviderSection(
      "Questions to ask the provider",
      (guidance.whatToAsk || [guidance.technologyAsk || "What coverage, ownership, reporting, and evidence will you provide?"])
        .flatMap((item: string) => wrapText(`- ${item}`, 90))
        .slice(0, 8)
    );

    const evidenceItems = [
      guidance.checklist?.outcome || "Evidence that the control is operating in practice.",
    ...
    drawProviderSection(
      "Evidence to request",
      evidenceItems.flatMap((item: string) => wrapText(`- ${item}`, 90)).slice(0, 8)
    );
  });

  if (y < 150) startTechPage(true);

  page.drawText("Important buying note", { x: 50, y, size: 11, font: fontBold, color: BRAND.text });
  y -= 16;

  const note =
    "The right question is not 'which vendor is best?' but 'which capability, coverage, owner, reporting, and evidence do we need based on our weakest areas?' Use this section to brief providers clearly before buying tools.";
